...
class InfiniteGridMenu {
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.discProgram = createProgram(
      ...
      {
        aModelPosition: 0,
        aModelNormal: 1, // no se usa en el código, para mantener la ubicación
        aModelUvs: 2,
        aInstanceMatrix: 3,
      },
    );
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // No destruimos la instancia al desmontar
    };
  }, [items]); // Solo se ejecuta cuando cambian los items
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }} className="bg-space-dark rounded-xl overflow-hidden">
      {/* Elementos decorativos similares a SpaceSection */}
      ...
      {activeItem && (
        <h2 className={`face-title ${isMoving ? "inactive" : "active"}`}>
          {activeItem.title
            .split(' ')
            .reduce((acc: string[], word: string, i: number, arr: string[]) => {
              // Lista de palabras cortas que queremos mantener en la misma línea
              const shortWords = ['de', 'la', 'el', 'los', 'las', 'del'];
              
              if (shortWords.includes(word.toLowerCase()) && i < arr.length - 1) {
                // Si es una palabra corta y no es la última, la unimos con la siguiente
                acc.push(`${word} ${arr[i + 1]}`);
                // Saltamos la siguiente palabra ya que la hemos incluido
                arr.splice(i + 1, 1);
              } else {
                acc.push(word);
              }
